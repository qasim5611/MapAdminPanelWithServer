...
import {
  deleteSidebar as deleteSidebarAction,
  UpdateSidebar,
...
const Footer = () => {
  ...
  const isSideBarDeleted = useSelector(
    (state) => state.sidebarReducer.isSideBarDeleted
  );
  ...
  useEffect(() => {
    axios.get(API + "/api/sidebar/get_all").then((result) => {
      setEditorList(result.data);
      setEditorList2(result.data);
    });
  }, [isSideBarDeleted, success]);

  const deleteSidebar = (id) => {
    dispatch(deleteSidebarAction({ _id: id, admin_id }));
  };
      ...
      <div class="content-wrapper">
        ...
        >
          ...
          <Row className="table-header">
            <Col xs="12" md="2">
              <h2 class="section-title">SideBar Section</h2>
            </Col>
            <Col xs="12" md="10">
                      ...
                      {editorList.map((item, ind) => {
                        return (
                          <tr>
                            ...
                            <td>
                              <Link to={`/admin/sidebar_edit/${item._id}`}>
                                <FontAwesomeIcon
                                  icon={faUserEdit}
